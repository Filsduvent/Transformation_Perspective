import cv2
import numpy as np
from utils import point_selector as utilities
...
class PaintingView:
    ...
    def get_contour(self, rescaled_image):
        ...
        if len(test_corners) >= 4:
            ...
            cv2.drawContours(rescaled_image,  [np.array(points, dtype=np.int32)], -1, (255, 0, 0), 2,)
            # save the transformed image
            # Create a generic basename with timestamp
            #basename = f"image_{datetime.now().timestamp()}"
            #filename = f'{basename}_contour.jpg'
            #output_path = os.path.join("painting_view_change/output/", filename)
    ...
    def transform(self, image_path):
        ...
        # Step 2: Select Points for Painting View

        points = self.get_contour(rescaled_image)

        # apply the perspective transformation
       

        if len(points) != 4:
            raise Exception("Exactly 4 points are required.")
        
        warped = utilities.four_point_transform_painting_view(orig, points *ratio)
        
        return rescaled_image, warped
